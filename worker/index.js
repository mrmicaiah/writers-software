// Writer's Dashboard - Cloudflare Worker with Alice
...
const ALICE_TOOLS = [
  ...
  { name: "create_character", description: "Create a character profile", input_schema: { type: "object", properties: { name: { type: "string" }, role: { type: "string", description: "protagonist, antagonist, supporting, minor" }, description: { type: "string" }, personality: { type: "string" }, backstory: { type: "string" }, arc: { type: "string" } }, required: ["name", "role"] } },
  { name: "create_chapter_outline", description: "Create a chapter outline with scenes", input_schema: { type: "object", properties: { chapter_number: { type: "number" }, title: { type: "string" }, summary: { type: "string" }, scenes: { type: "array", items: { type: "string" } }, pov: { type: "string" }, goals: { type: "string" } }, required: ["chapter_number", "summary"] } },
  ...
  try {
    ...
    switch (path) {
      ...
      case 'alice':
        ...
        break;
      case 'export': return await handleExport(env, url.searchParams.get('category'), url.searchParams.get('project'), url.searchParams.get('format'));
      default: return jsonResponse({ error: 'Not found' }, corsHeaders, 404);
    }
  } catch (err) { return jsonResponse({ error: err.message }, corsHeaders, 500); }
}
...
- No emojis.
- If flirted with, redirect gracefully back to the writing.`;
